...
- **Agent 配置管理与模型交互体验升级**：
  - **模型卡片一键复制**：Agent 配置表单中的模型卡片新增一键复制按钮，支持标准 Clipboard API 与非安全上下文自动回退，带 1.5s 交互反馈与定时状态自动重置；
  - **模型映射面板全量 SFC 迁移**：模型映射管理模块全面重构成 Vue 3 单文件组件（SFC），复用 `ModelSearchSelect` 公共组件，支持拼音与英文智能模糊搜索及高亮；
  - **Other 号池公共模型联动**：Other 号池完整接入公共模型选择器，组名修改实时联动底层中继服务器。

- **中继引擎与多客户端协议深度兼容**：
  - **ZCode 客户端思考与会话亲和性适配**：基于 User-Agent 智能区分 `ZCode` 与 `OpenCode` 客户端，精准标记 `zcode:` 前缀与会话亲和性；原生支持纯 `output_config.effort` 指令驱动深度思考；
  - **NVIDIA 链路 Claude Code 中断与死循环根治**：修复思考内容透传与 Tool ID 跨重试/跨轮一致性（`pinnedToolIDs`），杜绝工具调用中断引发的客户端死循环；
  - **多号池同目标模型路由解歧**：消除不同号池绑定同一目标模型时的路由匹配歧义；
  - **HTTP/2 显式协商**：出站客户端显式支持 HTTP/2 协议协商，提升并发吞吐。

- **系统性能、数据安全与稳定性优化**：
  - **UI 刷新与落盘解耦**：将 UI 状态更新节拍与磁盘持久化操作完全解耦，彻底消除快照并发读写竞态；
  - **请求日志 SQLite 存储迁移**：请求日志全面迁移至纯 Go SQLite 数据库存储，修复 `SaveToDisk` 潜在锁泄漏与大文件 I/O 阻塞；
  - **NVIDIA 对冲竞速增强**：增强对冲竞速逻辑与持久化防丢机制；
  - **前台工作集修剪防卡顿优化**：仅在后台托盘挂机时执行工作集修剪（`TrimProcessWorkingSet`），彻底解决前台使用时每 30 秒因工作集清空导致的缺页回盘卡顿。

- **工程架构高内聚模块化重构**：
  - 单体控制器大文件全面解耦，App 主控制器、IPC 消息分发（`app_ipc_*.go`）、生命周期与前端交互完成严密模块化拆分。
