...
import (
	...
	"antigravity-proxy/internal/platform/pkg/jwt"
...
func (s *AuthService) GetProfile(userID uint) (*model.User, error) {
	var user model.User
	err := db.GlobalDB.Preload("Plan").First(&user, userID).Error
	if err != nil {
		return nil, err
	}
	return &user, nil
...
func (s *AuthService) ChangePassword(userID uint, oldPwd, newPwd string) error {
	var user model.User
	if err := db.GlobalDB.First(&user, userID).Error; err != nil {
		return err
	}
	...
	return db.GlobalDB.Save(&user).Error
}
