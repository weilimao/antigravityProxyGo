...
func (s *SettingService) GetOCRModels() ([]string, error) {
	...
	err := db.GlobalDB.Where("`key` = ?", "ocr_models").First(&setting).Error
...
func (s *SettingService) GetOCRModel() (string, error) {
	var setting model.Setting
	err := db.GlobalDB.Where("`key` = ?", "ocr_model").First(&setting).Error
	if err != nil {
...
func (s *SettingService) SetOCRModels(models []string) error {
	...
	if err := db.GlobalDB.Save(&modelsSetting).Error; err != nil {
		return err
	}
	...
	if err := db.GlobalDB.Save(&modelSetting).Error; err != nil {
		return err
	}
...
func (s *SettingService) GetModelMappings() ([]model.ModelMappingEntry, error) {
	var setting model.Setting
	err := db.GlobalDB.Where("`key` = ?", "model_mappings").First(&setting).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (s *SettingService) SetModelMappings(mappings []model.ModelMappingEntry) error {
	...
	return db.GlobalDB.Save(&setting).Error
}
...
func (s *SettingService) GetAutoRacingConfig() (*model.AutoRacingConfig, error) {
	var setting model.Setting
	err := db.GlobalDB.Where("`key` = ?", "auto_racing_config").First(&setting).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (s *SettingService) SetAutoRacingConfig(cfg *model.AutoRacingConfig) error {
	...
	if err := db.GlobalDB.Save(&setting).Error; err != nil {
		return err
	}
...
func (s *SettingService) GetUserAutoConfig(userID uint) (*model.AutoRacingConfig, error) {
	...
	var setting model.Setting
	err := db.GlobalDB.Where("`key` = ?", userKey).First(&setting).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			// 回退全局默认配置
