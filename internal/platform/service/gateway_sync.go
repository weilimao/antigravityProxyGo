...
// FetchGatewayChannelModels 触发网关获取某个 Channel 的最新模型并返回新增列表及快照
func (s *GatewaySyncService) FetchGatewayChannelModels(channel string) (map[string]interface{}, error) {
	...
	if baseURL == "" {
		return nil, fmt.Errorf("GatewayURL not configured")
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("网关返回 HTTP %d: %s", resp.StatusCode, string(b))
	}

	var res map[string]interface{}
	if err := json.NewDecoder(resp.Body).Decode(&res); err != nil {
		return nil, fmt.Errorf("网关返回数据格式异常")
	}
	return res, nil
}

// FetchGatewayOtherGroupModels 触发网关获取 Other 号池某个组的最新模型
