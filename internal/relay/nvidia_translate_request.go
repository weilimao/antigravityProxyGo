...
func anthropicToOpenAIChat(req *AnthropicRequest, preserveImages bool, targetProvider string, mappings ...[]settings.ModelMappingEntry) (*OpenAIChatRequest, error) {
	...
	isOther := false
	if strings.EqualFold(strings.TrimSpace(targetProvider), "other") || strings.EqualFold(strings.TrimSpace(targetProvider), "workbuddy") || strings.EqualFold(strings.TrimSpace(targetProvider), "opencode") {
		isOther = true
