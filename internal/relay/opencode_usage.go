...
// opencode_usage.go: OpenCode Zen 号池的请求日志/统计落库链路。
//
// 与 workbuddy_usage.go / grok_usage.go / passthrough_usage.go 保持对偶形态:
//   - relay 维度模型名带 "opencode/" 前缀, 并在请求日志 Family="opencode" 显式入库;
//   - 号池成员账号维度(usage.json)去前缀展示上游真实模型名(claude-sonnet-4-6 等);
//   - 全局综合统计(TrackRequestForModel + TrackRequestForPool("opencode"))把 OpenCode 用量计入
//     顶部指标卡 / 模型统计表 / 综合趋势桶;
//   - 请求日志调用 AddRequestLogForFamily(reqLog), 使前端「使用详情」面板能完整展示请求明细。

var opencodeReqLogSeq uint64

type opencodeLogCtx struct {
	Method          string
	Host            string
	Path            string
	SessionID       string
...
func opencodeHostFromBaseURL(baseURL string) string {
...
// recordOpenCodeUsage 记录 OpenCode 一次成功请求的用量到各落点。
func (h *APICompatHandler) recordOpenCodeUsage(userSession *RelaySession, model string, input, output, cached int, poolAccount *account.Account, logCtx opencodeLogCtx) {
	...
	cleanModel = strings.TrimPrefix(cleanModel, "oc/")

	displayModel := model
	if !strings.HasPrefix(displayModel, "opencode/") && !strings.HasPrefix(displayModel, "oc/") {
		displayModel = "opencode/" + displayModel
	}
	...
	if usageAvailable && h.statsTracker != nil && userSession != nil {
		h.statsTracker.RecordUsage(RelaySample{
			ReqID:        fmt.Sprintf("oc-%d", time.Now().UnixNano()),
	...
	if h.globalStatsTracker != nil {
		...
		if poolAccount != nil {
			h.globalStatsTracker.TrackRequestForPool(displayModel, input, output, cached, "opencode")
		}

		// 4. 请求日志 (AddRequestLogForFamily, family="opencode")
		end := time.Now()
		...
		seq := atomic.AddUint64(&opencodeReqLogSeq, 1)
		relayUserID := ""
		if userSession != nil {
			relayUserID = userSession.UserKey
		}
		reqLog := &stats.RequestLog{
			ID:             fmt.Sprintf("oclog-%d-%d", seq, rand.Intn(1000)),
