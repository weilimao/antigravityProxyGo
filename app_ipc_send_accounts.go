...
func (a *App) handleAccountsSendIPC(channel string, args []interface{}) bool {
	...
	case "opencode:set-lb-mode":
		// args: [mode]。OpenCode 单池单值 LB 算法(与 workbuddy:set-lb-mode 同构)。
		mode := getStringArg(0)
		a.accountMgr.SetOpenCodeLBMode(mode)
		a.AddLog("🔄 OpenCode Load Balancing Algorithm switched to: " + a.accountMgr.GetOpenCodeLBMode())
		a.emitAccountsRes()
		return true
	...
	case "opencode:set-max-concurrency":
		// 单账号在途并发上限:0=未配置回退默认 10;超过自动换号(超额降级最少并发号)。
		v := getIntArg(0)
		a.accountMgr.SetOpenCodeMaxConcurrency(v)
