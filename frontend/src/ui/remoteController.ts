...
function updateRemoteStatusUI(status: any) {
    ...
    if (isConnected && isEnabled) {
        ...
        if (badge) {
            ...
            badge.setAttribute('title', isZH ? `远程主机: ${state.remoteHost}:${state.remotePort}\n用户Key: ${state.remoteUserKey}` : `Remote Host: ${state.remoteHost}:${state.remotePort}\nUser Key: ${state.remoteUserKey}`);
        }
        if (statusText) {
            statusText.textContent = isZH ? `远端: ${state.remoteHost}:${state.remotePort}` : `Remote: ${state.remoteHost}:${state.remotePort}`;
        }
