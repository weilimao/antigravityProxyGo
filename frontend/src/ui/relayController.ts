            ...
            <div class="p-4 rounded-xl border border-outline-variant/30 bg-white/50 dark:bg-[#1a2033] hover:border-primary/50 transition-all cursor-pointer shadow-sm flex flex-col justify-between group" onclick="window._relayOpenPackageSettings('${pkg.id}')">
                <div>
                    ...
                    <div class="space-y-2 text-[12px]">
                        ...
                        <div class="flex items-center justify-between bg-outline-variant/5 p-2 rounded-lg border border-outline-variant/10">
                            <span class="text-outline font-medium">${isZH ? '请求速率' : 'Rate Limit'}</span>
...
function renderRelayUsers() {
    ...
    container.innerHTML = relayUsers.map(user => {
        ...
        let expireStr = isZH ? '永久有效' : 'Lifetime';
        let durationStr = '';
        if (q.validDuration > 0) {
            const unitMap: any = isZH ? { days: '天', months: '个月', years: '年' } : { days: 'days', months: 'months', years: 'years' };
        ...
        if (q.expireAt > 0) {
            ...
            expireStr = isZH ? `${expDate.toLocaleDateString()} 到期` : `Expires at ${expDate.toLocaleDateString()}`;
            if (isExpired) {
                expireStr = isZH 
                    ? `<span class="text-red-500 font-bold">已于 ${expDate.toLocaleDateString()} 到期</span>` 
                    : `<span class="text-red-500 font-bold">Expired on ${expDate.toLocaleDateString()}</span>`;
            }
        }
...
(window as any)._relayRemoveUser = async (id: string) => {
    const dict = i18n[state.currentLanguage] || {};
    if (!await $confirm(dict.relayDeleteUserConfirm || '确定要删除该中继用户吗？')) return;
    try {
...
(window as any)._relayDeletePackage = async (id: string) => {
    const dict = i18n[state.currentLanguage] || {};
    if (!await $confirm(dict.relayDeleteTemplateConfirm || '确定要删除该套餐模板吗？')) return;
    try {
                        ...
                        if (!quota || (!quota.enableFixed && !quota.enableHourly && !quota.enableDaily)) {
                            return `
                                <div class="bg-white/50 dark:bg-white/5 p-3 rounded-lg border border-outline-variant/20 text-[12px] mb-3">
                                    <div class="font-semibold text-on-surface dark:text-white mb-1">${familyTitle} <span class="text-[10px] text-emerald-500 font-normal bg-emerald-500/10 px-2 py-0.5 rounded">${dict.relayNoLimit || '无限制'}</span></div>
