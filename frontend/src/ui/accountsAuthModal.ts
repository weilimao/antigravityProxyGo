    ...
    return new Promise((resolve) => {
        ...
        function showStep1And2() {
            card.innerHTML = `
                <div class="flex items-center gap-2 text-primary">
                    ...
                    <h3 class="text-base font-bold text-on-surface dark:text-white">${dict.authModalTitle || 'Google Cloud 账号授权'}</h3>
                </div>
                ...
                <div class="flex flex-col gap-3.5 my-1 text-[13px] text-on-surface dark:text-white">
                    <div class="text-[12px] text-outline leading-relaxed bg-slate-50 dark:bg-white/5 p-3.5 rounded-xl border border-outline-variant/20 flex flex-col gap-2.5">
                        <p>${dict.authStep1Text || '1. 点击下方按钮复制链接并在浏览器中打开，完成 Google 账户授权：'}</p>
                        <button id="flowOpenAuthLink" type="button" disabled class="w-full py-2.5 bg-slate-100 dark:bg-white/5 text-outline rounded-lg transition-all font-semibold text-[12px] border border-outline-variant/20 flex items-center justify-center gap-1.5 opacity-50 cursor-not-allowed">
                            ...
                            ${dict.gettingAuthLink || '正在获取官方授权链接...'}
                        </button>
                    </div>

                    <div class="flex flex-col gap-1.5">
                        <label class="text-[11px] text-outline font-medium">${dict.authStep2Text || '2. 将网页上重定向或显示的“授权码 (Authorization Code)”粘贴在下方：'}</label>
                        <input type="text" id="flowAuthCodeInput" placeholder="${dict.placeholderAuthCode || '输入以 4/ 开头的授权码...'}" class="w-full px-3 py-2 text-[13px] bg-slate-50 dark:bg-white/5 border border-outline-variant/30 rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-on-surface dark:text-white placeholder-outline/60" autofocus />
                    </div>
                ...
                <div class="flex justify-end gap-2 mt-2">
                    <button id="flowCancel" type="button" class="px-4 py-1.5 text-[12px] font-medium bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-on-surface dark:text-white rounded-lg transition-colors border border-outline-variant/40">${dict.btnCancel || '取消'}</button>
                    <button id="flowConfirm" type="button" class="px-4 py-1.5 text-[12px] font-bold bg-primary text-white hover:bg-primary/90 rounded-lg transition-colors shadow-sm flex items-center justify-center gap-1">${dict.btnConfirm || '开始登录'}</button>
                </div>
            `;
            ...
            ipcRenderer.invoke('auth:get-manual-oauth-url').then((authData: any) => {
                if (authData && authData.url) {
                    ...
                    btnOpen.innerHTML = `<span class="material-symbols-outlined text-[14px]">open_in_new</span> ${dict.btnOpenAuthBrowser || '复制链接并打开浏览器'}`;
                } else {
                    btnOpen.innerHTML = dict.getAuthLinkFailed || '获取授权链接失败，请重试';
                }
            }).catch((err: any) => {
                btnOpen.innerHTML = (dict.getAuthLinkError || '获取授权链接错误: {error}').replace('{error}', err.message);
            });
            ...
            btnOpen.addEventListener('click', () => {
                ...
                navigator.clipboard.writeText(cachedAuthData.url).then(() => {
                    ...
                    btnOpen.innerHTML = `<span class="material-symbols-outlined text-[14px]">done</span> ${dict.authLinkCopied || '已复制链接并打开浏览器'}`;
                    setTimeout(() => {
            ...
            async function handleConfirm() {
                ...
                if (!code) {
                    alert(dict.pleaseInputAuthCode || '请输入授权码 (Authorization Code)');
                    inputAuthCode.focus();
                    return;
                }
                if (!cachedAuthData) {
                    alert(dict.authLinkNotReady || '授权链接尚未准备好，请稍候');
                    return;
                }

                btnConfirm.disabled = true;
                btnConfirm.innerHTML = `<span class="material-symbols-outlined text-[14px] animate-spin">refresh</span> ${dict.btnVerifying || '校验中...'}`;
                ...
                try {
                    ...
                    btnConfirm.innerHTML = dict.btnConfirm || '开始登录';
                    inputAuthCode.disabled = false;
                    btnOpen.disabled = false;
                    divError.innerHTML = (dict.verificationFailed || '校验失败: {error}').replace('{error}', err.message);
        ...
        function showStep3(exchangeRes: any) {
            ...
            if (projects && projects.length > 0) {
                projectUIHtml = `
                    <div class="flex flex-col gap-1.5">
                        <label class="text-[11px] text-outline font-bold uppercase">${dict.selectGcpProjectLabel || '选择要绑定的 Google Cloud 项目：'}</label>
                        ...
                        <p class="text-[10px] text-outline leading-relaxed mt-0.5">${dict.selectProjectTip || '已从您的云端账户成功获取项目列表。请选择一个启用了 Gemini/Cloud AI Companion 的项目。'}</p>
                    </div>
                `;
            } else {
                ...
                projectUIHtml = `
                    <div class="flex flex-col gap-2.5">
                        <div class="text-[11px] bg-amber-500/10 text-amber-600 dark:text-amber-400 p-3 rounded-xl border border-amber-500/20 leading-relaxed flex items-start gap-1.5">
                            ...
                            <div class="break-all">
                                ${(dict.gcpProjectListFailed || '自动从云端获取项目列表失败{error}。由于谷歌官方 Client ID 的 API 限制，无法直接从云端列出您的项目。请在下方手动输入您的项目 ID。').replace('{error}', displayError)}
                            </div>
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label class="text-[11px] text-outline font-bold uppercase">${dict.inputGcpProjectIdLabel || '请输入您的 Google Cloud 项目 ID (Project ID)：'}</label>
                            ...
                            <p class="text-[10px] text-outline leading-relaxed mt-0.5">${dict.gcpProjectIdTip || '项目 ID 可以从 <a href="https://console.cloud.google.com" target="_blank" class="text-primary hover:underline">Google Cloud 控制台</a> 首页的“项目信息”中复制。请输入精确的 Project ID，否则拦截请求时会报错。'}</p>
                        </div>
                    </div>
                `;
            }
            ...
            card.innerHTML = `
                <div class="flex items-center gap-2 text-primary">
                    ...
                    <h3 class="text-base font-bold text-on-surface dark:text-white">${dict.bindGcpTitle || '绑定 GCP 项目'}</h3>
                </div>
                ...
                <div class="flex flex-col gap-3.5 my-1 text-[13px] text-on-surface dark:text-white">
                    <div class="text-[12px] text-outline leading-relaxed bg-slate-50 dark:bg-white/5 p-3.5 rounded-xl border border-outline-variant/20 flex flex-col gap-1.5">
                        <div class="flex justify-between">
                            <span class="text-outline">${dict.authorizedEmailLabel || '授权邮箱：'}</span>
                ...
                <div class="flex justify-end gap-2 mt-2">
                    <button id="flowCancel" type="button" class="px-4 py-1.5 text-[12px] font-medium bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-on-surface dark:text-white rounded-lg transition-colors border border-outline-variant/40">${dict.btnCancel || '取消'}</button>
                    <button id="flowSubmit" type="button" class="px-4 py-1.5 text-[12px] font-bold bg-primary text-white hover:bg-primary/90 rounded-lg transition-colors shadow-sm flex items-center justify-center gap-1">${dict.btnBindAndLogin || '确认绑定并登录'}</button>
                </div>
            `;
            ...
            async function handleSubmit() {
                ...
                if (!projectId) {
                    alert(dict.pleaseInputProjectId || '请输入或选择项目 ID');
                    if (inputProject) inputProject.focus();
                    return;
                }

                btnSubmit.disabled = true;
                btnSubmit.innerHTML = `<span class="material-symbols-outlined text-[14px] animate-spin">refresh</span> ${dict.btnBinding || '绑定中...'}`;
                ...
                try {
                    ...
                    btnSubmit.innerHTML = dict.btnBindAndLogin || '确认绑定并登录';
                    if (inputProject) inputProject.disabled = false;
                    if (selectProject) selectProject.disabled = false;
                    divSubmitError.innerHTML = (dict.saveFailed || '保存失败: {error}').replace('{error}', err.message);
