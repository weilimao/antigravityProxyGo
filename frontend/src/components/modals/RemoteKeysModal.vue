  ...
  <div class="hidden fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center" id="remoteKeysModal">
    <div class="bg-white dark:bg-[#1e2538] rounded-xl shadow-2xl w-[640px] p-6 border border-outline-variant/20 flex flex-col max-h-[80vh]">
      <div class="flex items-center gap-2 mb-5 flex-shrink-0">
        ...
        <h3 class="text-[16px] font-bold text-on-surface dark:text-white">管理 API Keys</h3>
      ...
      <div class="flex gap-2 mb-4 flex-shrink-0">
        <input class="flex-1 px-3 py-2 text-[13px] rounded-lg border border-outline-variant/30 bg-white dark:bg-[#1a1f30] text-on-surface dark:text-white focus:border-primary focus:ring-1 focus:ring-primary/30 outline-none" id="remoteNewKeyName" placeholder="新 Key 备注名称 (例如：Claude Code 测试端)" type="text">
        <button class="px-4 py-2 text-[12px] font-medium text-white bg-primary hover:bg-primary/90 rounded-lg transition-colors flex items-center gap-1 shadow-sm whitespace-nowrap" id="btnRemoteCreateKey">
          <span class="material-symbols-outlined text-[16px]">add</span> 创建 Key
        </button>
      </div>
        ...
        <table class="w-full text-left text-[12px]">
          <thead class="sticky top-0 bg-white dark:bg-[#1e2538] z-10">
            <tr class="border-b border-outline-variant/25 text-outline/80">
              <th class="py-2.5 font-bold pl-2 w-[110px]">名称</th>
              <th class="py-2.5 font-bold w-[160px]">API Key (脱敏展示)</th>
              <th class="py-2.5 font-bold w-[130px]">Gemini (已用/限额)</th>
              <th class="py-2.5 font-bold w-[130px]">Claude (已用/限额)</th>
              <th class="py-2.5 font-bold text-center w-[90px]">操作</th>
            </tr>
          </thead>
    ...
    <div class="bg-white dark:bg-[#1e2538] rounded-xl shadow-2xl w-[360px] p-5 border border-outline-variant/20 flex flex-col animate-in fade-in zoom-in-95 duration-150">
      <div class="flex items-center gap-2 mb-4 justify-between flex-shrink-0">
        <div class="flex items-center gap-1.5">
          ...
          <h3 class="text-[14px] font-bold text-on-surface dark:text-white" id="remoteQuotaEditTitle">修改 Key 限额</h3>
        </div>
      ...
      <div class="mb-4">
        <label class="block text-[11px] font-medium text-outline mb-1">Gemini 限额 Token 数</label>
        <input class="w-full px-3 py-2 text-[12px] rounded-lg border border-outline-variant/30 bg-transparent text-on-surface dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30" id="remoteQuotaEditGemini" placeholder="例如: 500k, 1m 或输入 0/留空不限制" type="text">
      </div>

      <div class="mb-4">
        <label class="block text-[11px] font-medium text-outline mb-1">Claude 限额 Token 数</label>
        <input class="w-full px-3 py-2 text-[12px] rounded-lg border border-outline-variant/30 bg-transparent text-on-surface dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30" id="remoteQuotaEditClaude" placeholder="例如: 500k, 1m 或输入 0/留空不限制" type="text">
      </div>
