...
import { revealFromBackend, nvidiaRevealAccountId, otherRevealAccountId, grokRevealAccountId, opencodeRevealAccountId } from '../../shared/revealKeyState';
...
const props = defineProps<{
  ...
  inputClass?: string;
  // 明文查看模式开关:
  // - revealProvider: "nvidia" | "other" | "grok" | "opencode"(对应后端 account:reveal-key 的 provider 校验);
...
watch([nvidiaRevealAccountId, otherRevealAccountId, grokRevealAccountId, opencodeRevealAccountId], () => {
  revealed = false;
  show.value = false;
});
...
function currentRevealAccountId(): string | null {
  ...
  if (props.revealProvider === 'opencode') return opencodeRevealAccountId.value;
  return null;
}
