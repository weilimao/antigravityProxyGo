<template>
  <BaseModal
    id="relayUserQuotaModal"
    hideType="hidden"
    zIndex="z-[9999]"
    icon="settings"
    maxWidth="w-[480px]"
    bodyClass="p-6 overflow-hidden flex flex-col"
  >
    <template #header-title>
      <span id="relayQuotaModalTitle" class="text-sm font-bold text-on-surface dark:text-white" data-i18n="relayQuotaModalTitleUser">限额配置</span>
    </template>
    ...
    <template #footer>
      <button class="px-4 py-2 text-[12px] font-medium text-outline hover:text-on-surface border border-outline-variant/30 rounded-lg transition-colors cursor-pointer" onclick="window._relayCloseModal('relayUserQuotaModal')" data-i18n="btnCancel">取消</button>
      <button class="px-4 py-2 text-[12px] font-medium text-white bg-primary hover:bg-primary/90 rounded-lg transition-colors cursor-pointer" onclick="window._relaySaveQuota()" data-i18n="btnConfirmText">保存</button>
    </template>
  </BaseModal>
</template>
