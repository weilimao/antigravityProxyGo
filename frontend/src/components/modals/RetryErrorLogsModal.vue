<template>
  <BaseModal
    id="retryErrorLogsModal"
    containerId="retryErrorLogsModalContainer"
    closeBtnId="retryErrorLogsModalCloseBtn"
    maxWidth="w-[950px] max-w-[95vw]"
  ...
  >
    ...
    <template #footer>
      <div class="flex justify-between items-center w-full">
        <button class="px-4 py-1.5 text-[12px] font-medium bg-rose-50 text-rose-600 hover:bg-rose-100 dark:bg-rose-950/20 dark:text-rose-400 dark:hover:bg-rose-950/40 rounded-lg transition-colors border border-rose-200/40 dark:border-rose-900/40 cursor-pointer" data-i18n="btnClearLogs" id="btnClearRetryErrorLogs">清空日志</button>
        <div class="flex gap-3">
          <button class="px-4 py-1.5 text-[12px] font-bold bg-primary text-white hover:bg-primary/90 rounded-lg transition-colors shadow-sm flex items-center gap-1 cursor-pointer" id="btnExportRetryErrorLogs">
          ...
          <button class="px-4 py-1.5 text-[12px] font-medium bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-on-surface dark:text-white rounded-lg transition-colors border border-outline-variant/40 cursor-pointer" data-i18n="btnClose" id="retryErrorLogsModalCloseBtnSecondary">关闭</button>
        </div>
      </div>
    </template>
  </BaseModal>
