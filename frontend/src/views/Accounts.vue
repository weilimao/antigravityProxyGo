...
<div class="flex flex-col gap-5 w-full" id="view-accounts">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 pb-1">
        ...
        <div class="flex flex-wrap items-center gap-2 md:gap-3">
            ...
            <!-- WorkBuddy 池负载均衡方式(仅 WorkBuddy 通道显示,与 nvidiaLBModeContainer 同构):轮询算法 select + 单账号在途并发上限 input -->
            <div class="flex items-center gap-2 bg-slate-50/50 dark:bg-white/5 px-3 py-1.5 rounded-lg border border-outline-variant/30 flex-shrink-0 hidden" id="workbuddyLBModeContainer">
                <span class="text-[13px] font-medium text-on-surface dark:text-white">轮询算法</span>
                <select class="bg-transparent text-[13px] font-medium text-on-surface dark:text-white border border-outline-variant/40 rounded px-1.5 py-0.5 focus:outline-none cursor-pointer" id="workbuddyLBModeSelect">
                    <option value="round-robin" class="dark:bg-[#1a1f30] text-slate-800 dark:text-white" data-i18n="lbRoundRobin">游标轮询 (默认)</option>
                    <option value="sticky" class="dark:bg-[#1a1f30] text-slate-800 dark:text-white" data-i18n="lbSticky">粘性会话</option>
                </select>
                <!-- WorkBuddy 池单账号在途并发上限:0=未配置回退默认 10;超过自动换号。 -->
                <div class="flex items-center gap-1 ml-1 pl-2 border-l border-outline-variant/20">
                    <span class="text-[12px] font-medium text-on-surface dark:text-white whitespace-nowrap" data-i18n="maxConcurrencyLabel">并发上限</span>
            ...
            <div class="relative flex-shrink-0">
                ...
                <div class="absolute right-0 mt-2 w-48 bg-white dark:bg-[#1a1f30] border border-outline-variant/30 rounded-xl shadow-xl py-2 hidden z-50" id="addAccountDropdown">
                    ...
                    <button class="w-full text-left px-4 py-2 text-[13px] text-on-surface dark:text-white hover:bg-slate-50 dark:hover:bg-white/5 transition-colors flex items-center gap-2 border-t border-outline-variant/10 mt-1 pt-3 cursor-pointer" id="btnAddWorkBuddyAccount" onclick="window.openWorkBuddyAccountModal && window.openWorkBuddyAccountModal()">
                        <span class="material-symbols-outlined text-teal-500 text-[16px]">support_agent</span>
                        <div>
