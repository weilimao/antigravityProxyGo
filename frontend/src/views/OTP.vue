...
<div>
<h1 class="text-2xl font-bold text-on-surface dark:text-white">2FA 验证码管理</h1>
<p class="text-xs text-outline dark:text-outline-variant">为账号配置谷歌两步验证 (2FA/TOTP) 密钥，实时生成并自动刷新动态验证码</p>
</div>
...
<span class="font-medium">验证码更新倒计时:</span>
<span class="font-bold text-primary dark:text-primary-fixed-dim w-6 text-center" id="otpCountdown">-</span>秒
                </div>
</div>
</div>
    <!-- 2FA 密钥即时查询 (不保存) -->
    <div class="glass-card rounded-xl p-4 flex flex-col gap-3">
        ...
        <div class="flex flex-col md:flex-row gap-3 items-stretch md:items-center">
            ...
            <div id="instantOtpResultContainer" class="hidden items-center gap-3 bg-emerald-500/10 dark:bg-[#10b981]/10 border border-emerald-500/20 dark:border-[#10b981]/20 rounded-xl px-4 py-2 select-none">
                ...
                <button id="btnCopyInstantOtp" class="material-symbols-outlined text-[16px] text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 cursor-pointer transition-colors" title="复制验证码">content_copy</button>
            </div>
            <div id="instantOtpError" class="hidden text-[12px] text-red-500 bg-red-500/10 px-4 py-2.5 rounded-xl border border-red-500/20">
                密钥格式无效
            </div>
        </div>
    </div>
    ...
    <div class="glass-card rounded-xl flex flex-col flex-1 min-h-[300px]">
        <div class="p-4 border-b border-outline-variant/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-slate-50/50 dark:bg-white/5 rounded-t-xl">
            <div class="flex items-center gap-3 w-full sm:w-auto">
                <span class="text-[13px] font-bold text-outline dark:text-outline-variant whitespace-nowrap">账号 2FA 列表</span>
            ...
            <div class="flex items-center gap-2 w-full sm:w-[260px] relative">
                <input type="text" id="otpSearchInput" placeholder="按邮箱筛选..." class="w-full pl-8 pr-3 py-1.5 bg-slate-100 dark:bg-white/5 border border-outline-variant/30 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-[12px] text-on-surface dark:text-white placeholder-outline/50" />
        ...
        <div class="flex-grow overflow-y-auto overflow-x-auto">
            <table class="w-full text-left border-collapse text-[13px] min-w-[600px]" id="otpTable">
                <thead>
                    <tr class="border-b border-outline-variant/50 bg-slate-50/50 dark:bg-white/5 text-outline select-none">
                        <th class="p-4 w-1/3 text-[11px] font-bold uppercase tracking-wider">账号邮箱</th>
                        <th class="p-4 w-1/4 text-[11px] font-bold uppercase tracking-wider">2FA 状态 / 密钥</th>
                        <th class="p-4 w-1/5 text-[11px] font-bold uppercase tracking-wider text-center">动态验证码</th>
                        <th class="p-4 w-1/5 text-[11px] font-bold uppercase tracking-wider text-right">操作</th>
                    </tr>
                </thead>
            ...
            <div class="hidden flex-col items-center justify-center py-12 text-outline/50" id="otpEmptyState">
                ...
                <span class="text-[13px]">暂无账号数据</span>
            </div>
        </div>
        <!-- 分页控制栏 -->
